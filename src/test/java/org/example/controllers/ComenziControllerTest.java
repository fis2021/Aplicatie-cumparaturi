...
// System.out.println(App.getCos());
        scene = new Scene(loadFXML("Comenzi.fxml"), 950, 700);
    ...
    void testCautaComanda(FxRobot robot){
        ...
        assertThat(robot.lookup("#mesaj2").queryText()).hasText("Comanda gasita!");
    }
    ...
    void testCautaComanda2(FxRobot robot){
        ...
        robot.clickOn("#cautaButton");
        assertThat(robot.lookup("#mesaj2").queryText()).hasText("Comanda inexistenta!");
    }

    @Test
    @DisplayName("Verify salveazaComanda method - successful/fail scenario")
    void testSalveazaComanda(FxRobot robot){
        //successful
        robot.clickOn("#idComanda").write("c1#0");
        robot.clickOn("#cautaButton");
        assertThat(robot.lookup("#mesaj2").queryText()).hasText("Comanda gasita!");
        robot.clickOn("#mesaj").write("Maine predam marfa la curier!");
        robot.clickOn("#accept");
        robot.clickOn("#salveazaButton");
        assertThat(robot.lookup("#mesaj1").queryText()).hasText("Comanda salvata cu succes!");
        assertThat(ComandaService.getAllComanda().get(0).getMesaj()).isEqualTo("Maine predam marfa la curier!");
        assertThat(ComandaService.getAllComanda().get(0).getAcceptare()).isEqualTo("Acceptata");
        // assertThat(ComandaService.getAllComanda().get(0))
        //fail but at least success ;)
        robot.clickOn("#idComanda").eraseText(4);
        robot.clickOn("#idComanda").write("c1#1");
        robot.clickOn("#mesaj").eraseText(29);
        robot.clickOn("#mesaj").write("Mesaj de proba!");
        robot.clickOn("#accept");
        robot.clickOn("#salveazaButton");
        robot.clickOn("#decline");
        robot.clickOn("#salveazaButton");
        assertThat(robot.lookup("#mesaj1").queryText()).hasText("Comanda salvata cu succes!");
//        assertThat(ComandaService.getAllComanda().get(1).getMesaj()).isEqualTo("Produsul numai e pe stoc!");
        // assertThat(ComandaService.getAllComanda().get(1).getAcceptare()).isEqualTo("Respinsa");
    }
    @Test
    @DisplayName("Verify salveazaComanda method - a fail scenario")
    void testSalveazaComanda2(FxRobot robot){
        robot.clickOn("#idComanda").write("c1#0");
        robot.clickOn("#cautaButton");
        robot.clickOn("#decline");
        robot.clickOn("#salveazaButton");
        assertThat(robot.lookup("#mesaj2").queryText()).hasText("Completati mesajul!");
    }
}
